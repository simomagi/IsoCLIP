...
class CocoDataset(Dataset):
    def __init__(self, dataroot: Union[str, Path], split: str, preprocess: callable, return_image=False):
        ...
        for idx, annotation in enumerate(annotations):
            ...
            self.flat_captions.extend([caption['raw'] for caption in annotation['sentences']])
    ...
    def get_caption(self, caption_idx: int) -> str:
        """Return the *original* raw sentence corresponding to a text-label index."""
        return self.flat_captions[caption_idx]
...
if __name__ == '__main__':
    ...
    dataset = CocoDataset('/andromeda/datasets', 'train', preprocess, return_image=True)
